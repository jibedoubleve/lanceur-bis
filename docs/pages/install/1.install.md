...
The easiest way to install Lanceur is via [winget](https://learn.microsoft.com/en-us/windows/package-manager/winget/):
...
- 1. **Start the application via a scheduled task** (Experimental Feature): The installer will create a scheduled task that launches Lanceur when the user logs in. *Note: This method has a known issue where the application closes when the computer switches to battery power.*
- 2. **Start the application via the registry**: Lanceur will be added to the registry to start automatically at login.
