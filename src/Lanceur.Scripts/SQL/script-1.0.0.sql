...
delete from alias_usage where id_session > (select min(id)from alias_session);
...
insert into alias_usage_temp(id, id_alias, time_stamp)
                      select id, id_alias, time_stamp from alias_usage;

drop table alias_usage;
alter table alias_usage_temp rename to alias_usage;

----------------------------------------
---- TABLE ALIAS                    ----
----------------------------------------
/* Remove all usage of session other than main one*/
delete from alias where id_session > (select min(id)from alias_session);
...
insert into alias_temp(id, arguments, file_name, notes, run_as, start_mode, working_dir, icon, hidden, lua_script, thumbnail, exec_count, confirmation_required)
                select id, arguments, file_name, notes, run_as, start_mode, working_dir, icon, hidden, lua_script, thumbnail, exec_count, confirmation_required from alias;

drop table alias;
alter table alias_temp rename to alias;
----------------------------------------
---- TABLE ALIAS_SESSION            ----
----------------------------------------
