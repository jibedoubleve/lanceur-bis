...
internal static class ProcessExtensions
{
    ...
    public static (string FileName, string Descripiton) GetInformation(this Process process)
    {
        try
        {
            ...
            if (hProcess == IntPtr.Zero)
            {
                throw new NotSupportedException(
                    $"Cannot open process {process.ProcessName} (PID: {process.Id}), error: {Marshal.GetLastWin32Error()}"
                );
            }
            ...
            {
                throw new NotSupportedException(
                    $"Cannot get process path for {process.ProcessName} (PID: {process.Id}), error: {Marshal.GetLastWin32Error()}"
                );
            }
        ...
        {
            throw new NotSupportedException($"Error processing process {process.ProcessName}: {ex.Message}", ex);
        }
    }
