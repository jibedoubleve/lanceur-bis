...
internal static class PackageInformation
{
    ...
    private enum Hresult : uint { Ok = 0x0000 }

    [Flags]
    private enum Stgm : uint { Read = 0x0, DenyWrite = 0x20 }
    ...
    private static string Get(Package package, Func<AppxPackageHelper.IAppxManifestApplication, string> visitor)
    {
        ...
        var hResult = SHCreateStreamOnFileEx(path, exclusiveRead, noAttribute, false, null, out var stream);
    ...
    {
        var versionFromNamespace = new Dictionary<string, PackageVersion> { { "http://schemas.microsoft.com/appx/manifest/foundation/windows10", PackageVersion.Windows10 }, { "http://schemas.microsoft.com/appx/2013/manifest", PackageVersion.Windows81 }, { "http://schemas.microsoft.com/appx/2010/manifest", PackageVersion.Windows8 } };
    ...
    {
        var logoKeyFromVersion = new Dictionary<PackageVersion, string> { { PackageVersion.Windows10, "Square44x44Logo" }, { PackageVersion.Windows81, "Square30x30Logo" }, { PackageVersion.Windows8, "SmallLogo" } };
        if (logoKeyFromVersion.ContainsKey(version))
        {
            var key = logoKeyFromVersion[version];
            app.GetStringValue(key, out var logoUri);
            return logoUri;
        }
    ...
    internal static string LogoPathFromUri(string uri, string location, string theme, PackageVersion version)
    {
        ...
        if (extension != null)
        {
            ...
            paths = paths.OrderByDescending(x => x.Contains(theme)).ToList();
            var selectedIconPath = paths.OrderBy(
                                            x =>
                                            {
