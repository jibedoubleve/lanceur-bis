...
public partial class PluginsView : IViewFor<PluginsViewModel>
{
    ...
    public PluginsView()
    {
        ...
        this.WhenActivated(
            d =>
            {
                ViewModel.AskFile.RegisterHandler(
                    interaction =>
                    {
                        var ofd = new OpenFileDialog { Title = "Select the plugin package", Filter = "Plugin package|*.lpk|All files|*.*" };
                        var result = ofd.ShowDialog();
                        var file = result.HasValue && result.Value ? ofd.FileName : string.Empty;
                        interaction.SetOutput(file);
                    }
                );
                ViewModel.AskWebFile.RegisterHandler(
                    interaction =>
                    {
                        var view = new PluginFromWebView { Owner = Window.GetWindow(this), Interaction = interaction };
                        view.ShowDialog();
                    }
                );
                ...
                {
                    plugin?.ConfirmRemove.RegisterHandler(
                        async interaction =>
                        {
                            var result =
