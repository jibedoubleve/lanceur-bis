...
public class ExecutableWithResultsTestAlias : MacroQueryResult
{
    ...
    public static ExecutableWithResultsTestAlias FromName(string name) => new() { Name = name, Query = new(name) };

    public override SelfExecutableQueryResult Clone() => this.CloneObject();

    public override Task<IEnumerable<QueryResult>> ExecuteAsync(Cmdline cmdline = null)
    {
        var result = new List<QueryResult>() { FromName("name1"), FromName("name2") };
