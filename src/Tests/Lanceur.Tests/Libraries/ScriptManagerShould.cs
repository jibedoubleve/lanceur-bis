...
public class ScriptManagerShould
{
    ...
    [Theory, InlineData("0.1", 2), InlineData("0.2", 1), InlineData("0.2.1", 0)]
    public void ExecuteExpectedScriptAfterGivenVersion(string version, int expectedCount)
    {
        ...
        foreach (var script in scripts.After(ver)) counter++;
    ...
    [Theory, InlineData("0.1"), InlineData("0.2"), InlineData("0.2.1")]
    ...
    [Theory, InlineData("1.0.0", 2), InlineData("1.0.1", 1), InlineData("1.1.1", 0)]
    public void UpdateAfter(string version, int count)
    {
        var dico = new Dictionary<Version, string>() { { new Version("1.0.0"), "" }, { new Version("1.0.1"), "" }, { new Version("1.1.1"), "" } };
