...
public class PluginDirectoryShould
{
    ...
    public PluginDirectoryShould(ITestOutputHelper output) => _output = output;

    #endregion Constructors

    #region Methods

    [Theory, InlineData(true, "directory/plugin.dll"), InlineData(true, "/directory/plugin.dll"), InlineData(false, "directory"), InlineData(false, "directory/"), InlineData(false, "/directory")]
    public void BeInMyDocuments(bool isFile, string dir)
    {
        ...
        if (isFile) expectedDirectory = Path.GetDirectoryName(expectedDirectory);
